...
import { addPartner, db, updatePartner } from "../../firebase/firestore";
...
function PartnerForm() {
    ...
    async function handleSubmit(event) {
        ...
        if (hasDuplicates(proposedActivities)) {
            ...
            if (fileChanged) {
                const stRef = ref(storage, "partnerFiles/" + partnerName);
                await listAll(stRef)
                    .then(res => {
                ...
                await updatePartner(partnerId, partnerType, partnerName, addressLine1, city, state, country, pincode, dmFirstName,
    ...
    React.useEffect(() => {
        ...
        const q2 = query(doc(db, "partnerData", partnerId));
        onSnapshot(q2, (snap) => {
            setPartnerType(snap.data().type);
            setPartnerName(snap.data().name);
            setAddressLine1(snap.data().address.addressLine1);
            setCity(snap.data().address.city);
            setState(snap.data().address.state);
            setCountry(snap.data().address.country);
            setPincode(snap.data().address.pincode);
            setDMFirstName(snap.data().decisionMaker.firstName);
            setDMLastName(snap.data().decisionMaker.lastName);
            setDMEmail(snap.data().decisionMaker.email);
            setDMPhone(snap.data().decisionMaker.phone);
            setContactPersons(snap.data().contactPersons);
            setCPCount(snap.data().contactPersons.length);
            setProposedActivities(snap.data().proposedActivities);
            setProposedActivitiesCount(snap.data().proposedActivities.length);
            setProjectsValue(snap.data().projects);
            setProjectsCount(snap.data().projects.length);
            setFileURL(snap.data().mouFile);
        });
    }, []);
                ...
                <div className="formContainer">
                    <div className="row">
                        <div className="column">
                            <label htmlFor=""><strong>Kind of Partner:</strong></label>
                            <select name="partnerType" id="partnerType" className="form-inp" placeholder="Enter the kind of partner" value={partnerType} onChange={handleChange}>
                        ...
                        <div className="column">
                            ...
                            <input type="text" name="partnerName" id="partnerName" className="form-inp" placeholder="Enter the name of partner" value={partnerName} onChange={handleChange} />
                        </div>
                    </div>
                </div>
                <div className="formContainer" id="mobilescreen-studentreport">
                ...
                <div className="formContainer">
                    <label style={{fontWeight: "600"}}><strong>Contact Persons Details:</strong> </label>
                    {
                        ...
                        <div className="column">
                                    ...
                                    <input type="email" name={"cpEmail"+index} id={"cpEmail"+index} placeholder="Enter the email" className="form-inp" value={contactPersons[index].email} onChange={mfHandleChangeCPEmail}/>
                                </div>
                                <div className="column">
