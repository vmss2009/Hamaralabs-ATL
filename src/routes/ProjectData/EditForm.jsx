...
import {updateProject, db} from "../../firebase/firestore";
...
function ProjectForm() {
    ...
    async function handleSubmit(event) {
        ...
        if(hasDuplicates(teamMembers) || hasDuplicates(tags)) {
            ...
            await updateProject(projectID, projectName, description, expectedStartDate, expectedEndDate, tags, teamMembers, tasks)
                .then(() => {
    ...
    React.useEffect(() => {
        const q = query(doc(db, "projectsData", projID));
        console.log(projID)
        onSnapshot(q, snap => {
            const temp = snap.data();
            setProjectID(temp.projectId);
            setProjectName(temp.projectName);
            setDescription(temp.projectDescription);
            setExpectedStartDate(temp.expectedStartDate);
            setExpectedEndDate(temp.expectedEndDate);
            setTags(temp.tags);
            setTeamMembers(temp.projectMembers);
            setTasks(temp.tasks);
            setTagsCount(temp.tags.length);
            setTeamMembersCount(temp.projectMembers.length);
            setTasksCount(temp.tasks.length);
        });
    }, []);
    ...
    document.title = "Project Data Edit | Digital ATL";
            ...
            <form>
                ...
                <div className="formContainer">
                    ...
                    <div className="column">
                        ...
                        <br/>
                        <br/>
                        <textarea name="description" id="description" className="form-inp" placeholder="Enter the Description" cols="30" rows="5" value={description} onChange={handleChange}/>
                    </div>
                    </div>
                    </div>
                    <div className="formContainer">
                                            ...
                                            </div>
                                            {
                                                createArray(tasks[index].subTasksCount).map((_, index2) => {
                                                    return <div className="formContainer" key={index2} style={{display: "inline-block"}}>
                                                       <strong> Sub Task {index2 + 1}: Name:</strong> <input type="text" name={"task"+index+"SubTask"+index2} id={"task"+index+"SubTask"+index2} data-task={index} data-subTask={index2} value={tasks[index].subTasks[index2].name} onChange={mfHandleChangeSubTaskName} className="form-inp"/>
                                                        <br/><br/> <strong>Description:</strong> <br/><br/> <textarea name={"task"+index+"SubTask"+index2+"Description"} id={"task"+index+"SubTask"+index2+"Description"} className="form-inp" cols="30" rows="5" data-task={index} data-subTask={index2} value={tasks[index].subTasks[index2].description} onChange={mfHandleChangeSubTaskDescription}/>
                                                    </div>
                                                })
                                            }
                                            <br/>
