...
import { db, updateMentor } from "../../firebase/firestore";
...
function Form() {
    ...
    async function handleSubmit(event) {
        ...
        // await updateMentor(mentorId, collegeName, fName, lName, mentorStudy, mentorDepartment, mentorYear, mentorEmail, whatsappNumber, aspiration, currentExperiment, competitionMapped)
        await updateMentor(mentorId, fName, lName, skillSet)
            .then(() => {
    ...
    React.useEffect(() => {
        const q = query(doc(db, "mentorData", mentorId));
        onSnapshot(q, (snap) => {
            // setAspiration(snap.data().aspiration);
            // setCollegeName(snap.data().college);
            // setCompetitionMapped(snap.data().competitionMapped);
            // setCurrentExperiment(snap.data().currentExperiment);
            // setMentorDepartment(snap.data().department);
            // setMentorEmail(snap.data().email);
            setFName(snap.data().name.firstName);
            setLName(snap.data().name.lastName);
            setSkillSet(snap.data().skillSet);
            setSkillsLength(snap.data().skillSet.length);
            // setMentorStudy(snap.data().study);
            // setWhatsappNumber(snap.data().whatsappNumber);
            // setMentorYear(snap.data().year);
        });
    }, [mentorId]);
    ...
    document.title = "Mentor Data Update | Digital ATL";
    ...
    return (
        <div className="container" id="mobilescreen">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="school-title">Mentors Data Update</div>
                <hr />
