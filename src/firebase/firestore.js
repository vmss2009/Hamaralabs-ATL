...
async function deleteGeneratedActivity(docId) {
    const docRef = doc(db, generatedActivityPath, docId);
    const snap = await getDoc(docRef);
    const archiveRef = doc(db, "archivedData", "appData", generatedActivityPath, docId);
    await setDoc(archiveRef, snap.data());
    await deleteDoc(docRef);
}

async function addStudent(fname, lname, school, gender, classVal, section, aspiration, isTeamLeader, comments,/* currentActivity,*/ email, tags) {
    try {
...
async function updateStudent(id, fname, lname, school, gender, classVal, section,  aspiration, isTeamLeader, comments, email, tags) {
...
async function addApartment(name,address,city,state,pincode,inchargeFName,inchargeLName,inchargeEmail,inchargeWhatsappNumber) {
    const docRef = collection(db, apartmentPath);
    await addDoc(docRef, {
        name: name,
        address: {
            addressLine1: address,
...
async function getApartment(apartmentId) {
    const docRef = doc(db, apartmentPath, apartmentId);
    const docSnap = await getDoc(docRef);
    return docSnap;
}

async function updateApartment(apartmentId, name,address,city,state,pincode,inchargeFName,inchargeLName,inchargeEmail,inchargeWhatsappNumber) {
    const docRef = doc(db, apartmentPath, apartmentId);
    await updateDoc(docRef, {
        name: name,
        address: {
...
async function deleteApartment(docId) {
    console.log(docId);
    const docRef = doc(db, schoolPath, docId);
    const snap = await getDoc(docRef);
    const archiveRef = doc(db, "archivedData", "appData", apartmentPath, docId);
    await setDoc(archiveRef, snap.data());
    await deleteDoc(docRef);
}

async function queryApartment(field, comparision, value) {
    const ref = collection(db, apartmentPath);
    const q = await query(ref, where(field, comparision, value));
    const querySnapshot = await getDocs(q);
    return querySnapshot;
}
...
async function task(taskName, taskDescription, taskDueDate, taskDone, taskComments) {
    try {
        ...
        for (const user of users) {
            ...
            if (docData.data().tasks === undefined) {
                await setDoc(userDocRef, {
                    tasks: [{taskName: taskName, taskDueDate: taskDueDate, ref: doc(db, "tasksData", docSnap.id)}]
                }, {merge: true});
            } else {
                await setDoc(userDocRef, {
                    tasks: [...docData.data().tasks, {
                        taskName: taskName,
                        taskDueDate: taskDueDate,
                        ref: doc(db, "tasksData", docSnap.id)
                    }]
                }, {merge: true});
            }
        }
...
async function taskAssign(taskId, user) {
    try {
        ...
        const users = [...taskData.data().users];
        users.push(userDocRef);
        tasks.push({
            ref: taskRef,
...
async function deleteTask(docId) {
    ...
    if (taskData.data().assignedTo !== undefined && role === "student") {
        return "cannot-delete: Permission Denied";
    }
    ...
    for (const userRef of users) {
        ...
        tasks = tasks.filter(task => task.ref.id !== docId);
...
async function getTags() {
    const docRef = collection(db, tagsPath);
    const docSnaps = await getDocs(docRef);
...
export default db;
export { db };
export { deleteGeneratedActivity };
...
export { addApartment, getApartments, getApartment, updateApartment, deleteApartment, queryApartment };
